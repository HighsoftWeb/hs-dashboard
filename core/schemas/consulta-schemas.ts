...
export const schemaFiltroClienteFornecedor = schemaParametrosConsulta.extend({
  sit: z.preprocess((val) => (val === null || val === "" ? undefined : val), z.enum(["A", "I"]).optional()),
  tipo: z.preprocess(
...
export const schemaFiltroProduto = schemaParametrosConsulta.extend({
  sit: z.preprocess((val) => (val === null || val === "" ? undefined : val), z.string().optional()),
  ind: z.preprocess((val) => (val === null || val === "" ? undefined : val), z.string().optional()),
});

export const schemaFiltroOrcamentoOS = schemaParametrosConsulta.extend({
  ind: z.preprocess((val) => (val === null || val === "" ? undefined : val), z.enum(["OR", "OS"]).optional()),
  sit: z.preprocess((val) => (val === null || val === "" ? undefined : val), z.string().optional()),
  codCliFor: z.preprocess(
    (val) => {
...
export const schemaFiltroTitulo = schemaParametrosConsulta.extend({
  ...
  dataInicio: z.preprocess((val) => (val === null || val === "" ? undefined : val), z.string().optional()),
  dataFim: z.preprocess((val) => (val === null || val === "" ? undefined : val), z.string().optional()),
});

export type ParametrosConsulta = z.infer<typeof schemaParametrosConsulta>;
export type FiltroClienteFornecedor = z.infer<typeof schemaFiltroClienteFornecedor>;
